...
```{r}
# Import the CSV file
df_just_stop_oil <- read_csv("dataframe_just_stop_oil.csv")
```
...
tokens_jso <- tokens(corpus, what = "word")
...
```{r}
# Assuming `jso_tokens_pp` is your preprocessed tokens
...
ggplot(sentiment_df, aes(x = sentiment)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean_sentiment, sd = sd_sentiment), color = "red", size = 1) +
  theme_minimal() +
  labs(title = "Distribution of Sentiment Scores for Just Stop Oil Protests Articles",
       x = "Sentiment Score",
       y = "Density")
