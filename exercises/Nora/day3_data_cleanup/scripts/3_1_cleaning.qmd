...
```{r}
cnn_data <- readRDS("C:/Users/chirikure/Desktop/Rprojects/SICSS_2024/sessions/day3_data_cleanup/data/CNN_cleaning.Rds")
```
...
cnn_new <- separate(cnn_data, authors, into = c("author1", "author2", "author3", "author4", "author5"), sep = ";", fill = "right")
...
cnn_new$status <- str_extract(cnn_new$timestamp, "\\b(Published|Updated)\\b")

library(lubridate)

cnn_new$weekday <- str_extract(cnn_new$timestamp, "\\b(EST|EDT), (\\w+)")
cnn_new$weekday <- str_extract(cnn_new$weekday, "\\w+$")

#first delete everything before the first space
cnn_new$date <- lubridate(cnn_new$timestamp)
