...
```{r}
text3 <- str_replace_all(text2, "Orange","")
print(text3)
```
...
```{r}
text4 <- str_replace_all(text3, "\\s+"," ")
text4 <- str_replace(text4,"^\\s|\\s$","")
print(text4)
```
...
```{r}
str_extract(text4, "\\w+\\.\\w+@\\w+\\.\\w+")
```

#### Change the text to all lower-case

```{r}
text5 <- str_to_lower(text4)
print(text5)
```
...
```{r}
text6 <- str_replace(text5, "impovemend","improvement")
print(text6)
