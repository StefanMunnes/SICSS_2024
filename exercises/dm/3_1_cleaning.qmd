...
# View the modified dataframe: click on df
```
...
data <- data.frame(
  observation = 1:5,
  timestamp = c("Published on 2023-07-01 12:34:56", 
                "Updated on 2023-07-02 13:45:07", 
                "Published on 2023-07-03 14:56:18", 
                "Updated on 2023-07-04 15:07:29", 
                "Published on 2023-07-05 16:18:40")
)

cnn_data <- cnn_data_seperated

# Clean and transform the timestamp column
cnn_data <- cnn_data %>%
  # Extract whether the article was published or updated
  mutate(status = ifelse(grepl("Published", timestamp), "Published", "Updated"),
         # Extract the date part from the timestamp and convert to Date format
         date = as.Date(sub(".* on ", "", substr(timestamp, 1, 19))),
         # Extract the weekday from the date
         weekday = weekdays(timestamp) %>%
  # Select only the relevant columns
  select(observation, status, weekday, date)
