...
```{r}
text <- str_replace_all(text, "[Oo]range", "")
print(text)
text_old <- text
print(text_old)
```
...
```{r}
mail <- str_extract(text, "\\w*?\\.\\w+@\\.*[a-z]+\\.[a-z]*")
print(mail)
print(text)
```
...
```{r}
text_corrected <- str_replace(text, "impovemend", "improvement")
...
```{r}
apple_pie_list <- str_which(texts, "apple pie")
print(apple_pie_list)
print(texts[apple_pie_list])
```

#### Find all sentences where "pie" is not directly preceded by "apple"

```{r}
apple_pie_not_list <- str_which(texts, "apple(?!pie)")
print(apple_pie_not_list)
print(texts[apple_pie_not_list])
```

#### Change "pineapple" to"apple" in all strings

```{r}
apple_pie_not_list <- str_replace_all(texts, "apple(?!pie)")
