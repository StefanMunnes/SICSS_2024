...
```{r}
file.edit("C:/Users/molitor/Documents/SICSS_2024/exercises/FM/.Renviron")
```
...
httr_names <- GET(url = "https://api.congress.gov/v3/member",
...
member <- content(httr_names, type='application/json') 

# We extracted three elements, but we only need the bills list
member <- content(httr_names, type='application/json')$member
...
```{r}
offsets <- seq(0, 2500, by = 250)

all_members_list <- lapply(offsets, function(offset) {
 GET(url = "https://api.congress.gov/v3/member",
     Sys.sleep(1),
                  query = list(format = "json",
                          limit = 250,
                          offset = offset,
...
# the name in the function 'offset' can be anything


# alternatively:
for (offset in offsets) {
  response <- GET(url = "https://api.congress.gov/v3/bill",
                  query = list(format = "json",
                          limit = 250,
                          offset = offset,
                           sort = "updateDate+desc",
  ...
  all_members_list <- append(all_members_list, response)
}
