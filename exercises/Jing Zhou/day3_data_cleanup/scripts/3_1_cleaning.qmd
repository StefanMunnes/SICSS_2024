...
```{r}
data_separated <- cnn_data %>%
  mutate(authors= str_replace_all(authors,"with",";"))%>%
  separate(authors, into = c("author1", "author2", "author3", "author4", "author5"), sep = ";", fill = "right")
...
```{r}
timestamp_discard <-cnn_data %>%
  mutate(
    # Identify if the article was updated or published
    status = if_else(str_detect(timestamp, "Updated"), "Updated", "Published"),
    
    # Extract the date and convert to Date format
    date = as.Date(parse_date_time(str_extract(timestamp, "\\w+ \\d{2}\\, \\d{4}"), orders = "mdy")),
    
    # Extract the weekday
    weekday = weekdays(date)
  ) %>%
  select(status, date, weekday)


# View the resulting dataset
print(timestamp_discard)
```
