...
```{r}
detect_line_break<-function(s){
    return(str_detect(s, "\n"))

}
print(detect_line_break(text))
```
...
remove_line_break<-str_remove(text,"\n")
...
```{r}
remove_Orange <- str_remove_all(remove_line_break,"Orange")
...
```{r}
remove_white_spaces <- str_replace_all(remove_Orange,"\\s+"," ")
...
```{r}
?count
number_character<-nchar(remove_white_spaces)

number_character

?str_count

number_word<-str_count(remove_white_spaces,"\\w+")
number_word
```
...
```{r}
?str_extract
email_address <- str_extract(remove_white_spaces, "\\w+.?\\w+@.+\\..+.")
email_address
```
...
```{r}
?str_to_lower
str_to_lower(remove_white_spaces, locale = "en")
...
# Find all sentences mentioning "apple" directly followed by "pie"
matches <- str_match_all(all_text, "\\bapple\\s+pie\\b")

# Extract matched sentences
matched_sentences <- unique(matches[[1]])
