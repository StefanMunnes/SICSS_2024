...
```{r}
# first try with logical operators that did not work:
text2 <- str_subset(text, !"\n") # Error in !"\n" : invalid argument type
...
```{r}
text3 <- str_replace(text2, "Orange", "")
```

#### Remove the unnecessary white spaces

```{r}
text4 <- str_replace(text3, "\\s{2,10}", "") # works but still unnecessary whitespaces
print(text4)
```
